/**
 * DogePad Arc Testnet 部署脚本 - 官方 RPC 版
 */
...
const ARC_CHAIN_ID = 5042002; // Arc Testnet 正确 Chain ID

// 使用更高的 gas 价格以加快确认
const GAS_PRICE = ethers.utils.parseUnits("25", "gwei"); // 25 Gwei
...
async function waitForConfirm(provider: ethers.providers.JsonRpcProvider, txHash: string, label: string) {
  ...
  // 等待最多 60 分钟
  for (let i = 0; i < 240; i++) { // 240 * 15秒 = 60分钟
    try {
    ...
    console.log(`  Still waiting... (${(i + 1) * 15}s / 60min)`);
...
async function main() {
  ...
  console.log("Connecting to Arc Testnet (official RPC)...");
