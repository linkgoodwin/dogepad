...
async function main() {
  ...
  console.log("  longPool:", existingAddresses.longPool);
  console.log("");
  ...
  const feeDistributor = await factory.deploy(
    ...
    { gasPrice: 2_000_000_000, gasLimit: 6_000_000 }
  );
  ...
  const tx1 = await bondingCurve.setFeeDistributor(feeDistributor.address, { gasPrice: 2_000_000_000, gasLimit: 500_000 });
  await tx1.wait();
  ...
  const tx2 = await launchDao.setFeeDistributor(feeDistributor.address, { gasPrice: 2_000_000_000, gasLimit: 500_000 });
  await tx2.wait();
